...
# Prefer the API virtualenv's python if present; fall back to python3 on PATH.
API_PY="${ROOT_DIR}/apps/api/.venv/bin/python"
if [[ ! -x "${API_PY}" ]]; then
  echo "warn: ${API_PY} not found; falling back to 'python3' on PATH." >&2
  echo "      Create it with: cd apps/api && python -m venv .venv && \\" >&2
...
(
  ...
  exec "${API_PY}" -m uvicorn app.main:app --host "${API_HOST}" --port "${API_PORT}"
) &
pids+=("$!")

echo "Starting web dev server (apps/web) ..."
(
